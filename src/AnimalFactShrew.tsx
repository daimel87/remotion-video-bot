...
const HEADLINE =
  "This desert shrew can survive **entire years without ever drinking water**. Living in some of the **hottest deserts on Earth**, it never needs a puddle or a stream — it pulls **almost all the water it needs straight from the food it eats**, squeezing moisture out of insects and prey. Its body is a **survival machine**: it barely sweats and wastes **almost no water at all**, enduring heat that would **kill most mammals in hours**. While other animals die of thirst, this tiny hunter just **keeps going**.";
...
// Flecha roja apuntando hacia abajo al shrew (el protagonista del video).
...
export const AnimalFactShrew: React.FC<{showFox?: boolean}> = ({showFox = false}) => {
  ...
  // Seguimiento del shrew (fracción del recuadro del video, 5s @ 24fps = 120f).
  // Corre desde la izquierda hasta el cráneo y se sube encima.
  const sx = interpolate(frame, [0, 36, 72, 108, 120], [37, 61, 63, 63, 63], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const sy = interpolate(frame, [0, 36, 72, 108, 120], [48, 44, 49, 51, 51], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#000'}}>
      ...
      >
        ...
        {/* Clip 16:9 (con su sonido ambiental) + flecha roja siguiendo al shrew */}
        <div style={{position: 'relative', width: '100%'}}>
          <OffthreadVideo
            src={staticFile('shrew-fact.mp4')}
