...
const HEADLINE =
  "This parasite swims into a fish's mouth, latches onto its tongue, and **drinks its blood until the tongue rots away and falls off**. Then it does something no other animal on Earth does: it **becomes the new tongue**. The fish uses the parasite to eat for the rest of its life — while it keeps **feeding on the fish from inside its mouth**. It's the **only known creature that replaces an organ of its host**, living there for **years**. And yes… they are **in the ocean right now**.";
...
// Flecha roja apuntando hacia abajo al parásito (el protagonista del video).
...
export const AnimalFactCymothoa: React.FC<{showFox?: boolean}> = ({showFox = false}) => {
  ...
  // Seguimiento del parásito (fracción del recuadro del video, 5s @ 24fps = 120f).
  // Se mantiene casi centrado dentro de la boca del pez.
  const px = interpolate(frame, [0, 36, 72, 108, 120], [50, 50, 48, 50, 50], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  const py = interpolate(frame, [0, 36, 72, 108, 120], [42, 41, 40, 38, 38], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  });
  ...
  return (
    <AbsoluteFill style={{backgroundColor: '#000'}}>
      ...
      >
        ...
        {/* Clip 16:9 (con su sonido submarino) + flecha roja siguiendo al parásito */}
        <div style={{position: 'relative', width: '100%'}}>
          <OffthreadVideo
            src={staticFile('cymothoa-fact.mp4')}
