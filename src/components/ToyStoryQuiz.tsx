  ...
  <div style={{
    ...
    {Array.from({length: total}).map((_, i) => (
      <div key={i} style={{
        ...
        boxShadow: i === current ? '0 0 15px rgba(255,215,0,0.6)' : 'none',
        transition: 'all 0.3s',
      }} />
    ))}
  </div>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: `conic-gradient(from ${rotation}deg at 80% 20%, transparent 0%, ${colors[0]}44 15%, transparent 30%)`,
      }} />
      <div style={{
        position: 'absolute', inset: -200,
        background: `conic-gradient(from ${rotation + 180}deg at 20% 80%, transparent 0%, ${colors[1]}44 15%, transparent 30%)`,
      }} />
      {/* Animated circles */}
      {[0, 1, 2, 3, 4].map((i) => {
        const x = 15 + i * 20;
        const y = 30 + Math.sin((frame + i * 40) * 0.04) * 20;
        const size = 150 + i * 60;
        return (
          <div key={i} style={{
            position: 'absolute',
...
const QuizVisuals: React.FC<{
  ...
  if (frame < INTRO_FRAMES) {
    ...
    const subtitleSlide = spring({frame: Math.max(0, frame - 15), fps, from: 80, to: 0, durationInFrames: 20});
    ...
    return (
      <AbsoluteFill>
        ...
        {[0, 1, 2, 3, 4, 5].map((i) => {
          const x = 10 + i * 16;
          const y = 20 + Math.sin((frame + i * 30) * 0.06) * 25;
          ...
          return (
            <div key={i} style={{
              position: 'absolute',
              left: `${x}%`, top: `${y}%`,
              fontSize: 180, fontWeight: 900, color: 'rgba(255,255,255,0.08)',
        ...
        <div style={{
          ...
          <div style={{
            fontSize: 160, transform: `scale(${titleScale})`, opacity: titleOpacity,
            filter: 'drop-shadow(0 8px 20px rgba(0,0,0,0.4))',
          }}>🎬</div>
          <div style={{
            fontSize: 130, fontWeight: 900, color: '#fff', textAlign: 'center',
          ...
          <div style={{
            fontSize: 52, color: '#fff', fontWeight: 800, opacity: subtitleOpacity,
            marginTop: 20,
    ...
    return (
      <AbsoluteFill>
        ...
        <div style={{
          ...
          <div style={{
            fontSize: 160, transform: `scale(${scale})`,
          ...
          <div style={{
            fontSize: 52, color: '#FFD700', fontWeight: 800,
  ...
  const pulseIntensity = isUrgent
    ? 1 + Math.sin(localFrame * 0.5) * 0.05
    : 1;

  const silhouetteBob = Math.sin(localFrame * 0.06) * 8;
  const urgentShake = isUrgent ? Math.sin(localFrame * 1.2) * 4 : 0;

  const numberColor = isReveal ? '#4ade80' : (countdownSeconds <= 3 ? '#ff4444' : '#FFD700');
  const numberScale = isReveal ? 0 : spring({
  ...
  return (
    <AbsoluteFill style={{overflow: 'hidden'}}>
      ...
      <div style={{
        ...
        borderRadius: 30, padding: '8px 30px',
        backdropFilter: 'blur(10px)',
      ...
      <div style={{
        position: 'absolute', top: 220, left: 0, right: 0, bottom: 320,
        ...
        <Img src={staticFile(ch.image)} style={{
          maxHeight: '100%', maxWidth: '75%', objectFit: 'contain',
        ...
        {!isReveal ? (
          <>
            ...
            <div style={{
              ...
              color: numberColor,
              textShadow: `0 0 60px ${numberColor}88, 0 8px 30px rgba(0,0,0,0.5)`,
            ...
            {/* Hint text */}
            <div style={{
              fontSize: 36, fontWeight: 700, color: 'rgba(255,255,255,0.5)',
