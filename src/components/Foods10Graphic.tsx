...
import {TomatoIcon, LeafIcon, MagnifierIcon} from './icons';
...
export const Foods10Graphic: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{opacity: exit}}>
      ...
      <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
        ...
        >
          Alimentos que frenan ese proceso
        </div>

        <div style={{display: 'flex', gap: 24}}>
          <Chip delay={20} icon={<TomatoIcon size={32} />}>
            Todos mexicanos
          </Chip>
          <Chip delay={40} icon={<LeafIcon size={32} />}>
            Todos accesibles
          </Chip>
          <Chip delay={60} icon={<MagnifierIcon size={32} />}>
            Todos con mecanismo
          </Chip>
        </div>
      </AbsoluteFill>
