...
SR=44100; DUR=76.8; N=int(SR*DUR)
...
# Buzz-SRT anchored (25fps). Hits ON card, no pre-riser. Final = warm chime.
cues=[
 (0/25,   impact(0.6), 0.0, 0.5),    # S1 title
 (110/25, blip(1500), 0.5, 0.5),     # S2 silencio
 (390/25, blip(1300),-0.4, 0.5),     # S3 evitar
 (540/25, blip(1300),0.4, 0.5),      # S4 miedo
 (680/25, blip(1300),-0.4, 0.5),     # S5 flujo
 (830/25, blip(1300),0.4, 0.5),      # S6 nocturnas
 (985/25, impact(0.6),0.0, 0.55),    # S7 evitacion (ON card)
 (1180/25,whoosh(0.4),0.0, 0.35),    # S7 exit
 (1300/25,impact(0.6),0.0, 0.55),    # S8 deteriora (ON card)
 (1440/25,whoosh(0.4),0.0, 0.35),    # S8 exit
 (1455/25,blip(1300),-0.4, 0.5),     # S9 no define
 (1610/25,chime(920),0.0, 0.45),     # S10 final positivo (ON card)
]
