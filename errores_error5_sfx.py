...
SR=44100; DUR=57.64; N=int(SR*DUR)
...
# Buzz-SRT anchored (25fps). Hits ON card, no pre-riser.
cues=[
 (0/25,   impact(0.6), 0.0, 0.5),    # S1 title
 (110/25, blip(1500), 0.5, 0.5),     # S2 relajarse
 (280/25, blip(1300),-0.4, 0.5),     # S3 depresor
 (360/25, impact(0.55),0.0, 0.5),    # S4 copas (ON card)
 (500/25, whoosh(0.4),0.0, 0.35),    # S4 exit
 (520/25, blip(1300),0.4, 0.5),      # S5 ereccion
 (800/25, impact(0.6),0.0, 0.55),    # S6 sin/con (ON card)
 (912/25, whoosh(0.4),0.0, 0.35),    # S6 exit
 (920/25, pop(640),  -0.4, 0.5),     # S7 trampa
 (1030/25,blip(1300),0.4, 0.5),      # S8 señal
 (1215/25,impact(0.55),0.0, 0.5),    # S9 siguiente error (ON card)
 (1290/25,impact(0.6),0.0, 0.55),    # S10 15 años (ON card)
]
