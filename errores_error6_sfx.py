...
SR=44100; DUR=56.88; N=int(SR*DUR)
...
# Buzz-SRT anchored (25fps). Hits ON card, no pre-riser. DUR padded past video for tail.
cues=[
 (0/25,   impact(0.6), 0.0, 0.5),    # S1 title
 (100/25, blip(1500), 0.5, 0.5),     # S2 sueño
 (210/25, blip(1300),-0.4, 0.5),     # S3 5 horas
 (290/25, impact(0.55),0.0, 0.5),    # S4 -15% (ON card)
 (395/25, whoosh(0.4),0.0, 0.35),    # S4 exit
 (405/25, impact(0.55),0.0, 0.5),    # S5 10-15 años (ON card)
 (560/25, whoosh(0.4),0.0, 0.35),    # S5 exit
 (585/25, blip(1300),0.4, 0.5),      # S6 cortisol
 (715/25, pop(640),  -0.4, 0.5),     # S7 antagonista
 (835/25, blip(1300),0.4, 0.5),      # S8 6 horas
 (990/25, impact(0.6),0.0, 0.55),    # S9 almohada (ON card)
 (1085/25,whoosh(0.4),0.0, 0.35),    # S9 exit
 (1100/25,impact(0.6),0.0, 0.55),    # S10 error 7 (ON card)
]
